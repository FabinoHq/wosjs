...
const skeletalMeshVertexShaderSrc = [
    ...
    "    return texture2D(lightsTexture, vec2(0.167, j)).x;",
    "}",
    "",
    "vec3 lightPos(float lightIndex)",
    "{",
    "    float j = (lightIndex+0.5)/lightsCount;",
    "    return texture2D(lightsTexture, vec2(0.167, j)).yzw;",
    "}",
    "",
    "void main()",
    "{",
...
const skeletalMeshFragmentShaderSrc = [
    ...
    "{",
    "    float j = (lightIndex+0.5)/lightsCount;",
    "    return texture2D(lightsTexture, vec2(0.5, j));",
    "}",
    "",
    "float lightRadius(float lightIndex)",
    "{",
    "    float j = (lightIndex+0.5)/lightsCount;",
    "    return texture2D(lightsTexture, vec2(0.832, j)).x;",
    "}",
    "",
    "float lightFalloffRadius(float lightIndex)",
    "{",
    "    float j = (lightIndex+0.5)/lightsCount;",
    "    return texture2D(lightsTexture, vec2(0.832, j)).y;",
    "}",
    "",
    "void main()",
    "{",
    ...
    "                float radius = lightRadius(float(i));",
    "                float falloffRadius = lightFalloffRadius(float(i));",
    ...
    "                float surfaceLen = length(surfaceLights[i]);",
    "                if (surfaceLen > radius)",
    "                {",
    "                    if (surfaceLen <= falloffRadius)",
    "                    {",
    "                        float fallatt = clamp((surfaceLen-falloffRadius)/",
    "                            (radius-falloffRadius), 0.0, 1.0",
    "                        );",
    "                        pointLight *= fallatt; specular *= fallatt;",
    "                    }",
    "                    else",
    "                    {",
    "                        pointLight = vec3(0.0, 0.0, 0.0); specular = 0.0;",
    "                    }",
    "                }",
